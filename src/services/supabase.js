...
export const profileService = {
  ...
  async checkListingLimit(userId) {
    ...
    if (error) throw error;
    return {
      canCreate: data.active_listings_count < data.max_listings_allowed,
      current: data.active_listings_count,
      max: data.max_listings_allowed
    };
  }
};
  ...
  async signUp(email, password, metadata = {}) {
    ...
    if (data.user) {
      await profileService.createProfile({
        ...
        profile_type: metadata.profile_type || 'personal',
        display_name: metadata.display_name || email.split('@')[0],
        max_listings_allowed: metadata.profile_type === 'business' ? 10 : 3
      });
    }
